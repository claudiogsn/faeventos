...
class EventoForm extends TPage
{
    ...
    public function __construct( $param )
    {
        ...
        $certificado_caminho = new TFile('certificado_caminho');


        // add the fields
        $this->form->addFields([new TLabel ('')],[$id]);
        $this->form->addFields( [ new TLabel('Nome :') ], [ $nome ] );
        $this->form->addFields( [ new TLabel('Data  de Inicio :') ], [ $dt_inicio ] );
        $this->form->addFields( [ new TLabel('Data de Encerramento :') ], [ $dt_fim ] );
        $this->form->addFields( [ new TLabel('Local :') ], [ $local ] );
        $this->form->addFields( [ new TLabel('Empresa Responsável :') ], [ $empresa_id ] );
        $this->form->addFields( [ new TLabel('Certificado :') ], [ $certificado_caminho ] );
        ...
        $id->setSize('100%');
        $nome->setSize('70%');
        $dt_inicio->setSize('40%');
        $dt_fim->setSize('40%');
        $local->setSize('40%');
        $empresa_id->setSize('40%');
        $certificado_caminho->setSize('40%');
